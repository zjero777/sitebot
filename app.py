...
def hot_lines():
    ...
    if filter_str is None:
        ...
        tab = Table_hotline.query.filter(
            Table_hotline.brand.ilike("%" + filter_str + "%")
        ).all()
    # jsonify({"data": {"val": val}})
    return render_template(
        "hot-lines.html", menu=main_menu, tab=tab, filter_str=filter_str
    )
...
def our_services():
    ...
    if request.method == "GET":
        return render_template(
            "our-services.html", menu=main_menu, service_list=service_list
        )
